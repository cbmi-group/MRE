...
import logging
from .base import  MS_PVT_SegModel_SingleLoss, MS_PVT_SegModel_Up_Sampling_Loss, \
    MS_PVT_SegModel_Multi_Layer_Loss, MS_PVT_SegModel_Hierarchical_Fusing_Loss
from mmengine.runner import load_checkpoint

from .multi_resolution_encoder import Multi_Resolution_Encoder
...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_classes=1, embed_dims=[64, 128, 256, 512],
...
class pvt_tiny(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        ...
        super(pvt_tiny, self).__init__(
            patch_size=2, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2],
            sr_ratios=[8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1)
...
encoder_types = {'pvt_large': pvt_large(),
...
class PVTMRE_SingleLoss(MS_PVT_SegModel_SingleLoss):
    def __init__(
            self,
            encoder_name: str = "pvt_small",
        ...
        self.encoder1 = encoder_types[encoder_name]
        self.encoder2 = Multi_Resolution_Encoder(n_channels=in_channels)


class PVTMRE_Up_Sampling_Loss(MS_PVT_SegModel_Up_Sampling_Loss):
    def __init__(
            self,
            encoder_name: str = "pvt_small",
            decoder_pyramid_channels: int = 256,
...
class PVTMRE_Multi_Layer_Loss(MS_PVT_SegModel_Multi_Layer_Loss):
    def __init__(
            self,
            encoder_name: str = "pvt_small",
            decoder_pyramid_channels: int = 256,
...
class PVTMRE_Hierarchical_Fusing_Loss(MS_PVT_SegModel_Hierarchical_Fusing_Loss):
    def __init__(
            self,
            encoder_name: str = "pvt_small",
            decoder_pyramid_channels: int = 256,
